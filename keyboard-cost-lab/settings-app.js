import { settingsScenario } from './data/settings-scenario.js';
import { settingsTasks } from './data/tasks-settings.js';
import { benchmarkResultsSettings } from './data/benchmark-results-settings.js';
...
const LAUNCH_STORAGE_PREFIX = 'keyboard-cost-lab-settings-launch';
const CHANNEL_PREFIX = 'keyboard-cost-lab-settings-channel';
const CHANNEL_FALLBACK_STORAGE_PREFIX = 'keyboard-cost-lab-settings-channel-fallback';
const SURVEY_CONFIG = {
  baseUrl: '',
};
...
const VARIANT_META = {
  variantA: {
    ...
    subtitle: '길게 이어진 설정 묶음을 차례로 지나야 하고, 각 설정의 설명 보기와 값 선택 버튼이 흩어져 있으며, 설명 대화상자를 닫으면 설정 화면 제목 근처부터 다시 찾아야 하는 구조',
    improvements: [
      '상단 보조 링크와 여러 설정 묶음을 차례로 지나야 원하는 설정에 도달합니다.',
      '각 설정마다 현재 상태, 변경 시각, 설명 보기, 값 선택 버튼이 따로 나뉘어 있어 순차 이동이 길어집니다.',
      '설명 대화상자를 닫으면 방금 보던 설정 대신 설정 화면 제목 근처로 돌아와 다시 위치를 찾아야 합니다.',
    ],
  },
  variantB: {
    shortLabel: 'B',
    title: '비교안 B · 개선 구조',
    subtitle: '설정 항목으로 바로 이동하고, 설정 묶음을 한 번 선택한 뒤 같은 묶음 안에서 필요한 설정을 바꾸며, 설명 대화상자를 닫으면 방금 누른 설명 보기 버튼으로 돌아오는 구조',
    improvements: [
      '설정 항목으로 바로 이동해 첫 진입 부담을 줄입니다.',
      '설정 묶음은 한 번만 들어간 뒤 방향키로 고를 수 있어 관련 없는 묶음을 길게 지나지 않습니다.',
      '설명 대화상자를 닫으면 같은 설명 보기 버튼으로 초점이 돌아와 다음 설정과 저장을 이어서 수행하기 쉽습니다.',
    ],
  },
};

const RUNNER_LABELS = {
  quickJump: '설정 항목으로 바로 이동',
  footerJump: '설정 항목으로 이동',
};

const GLOSSARY_ENTRIES = [
  {
    term: '비교안 A/B',
    description: '같은 설정 과업을 두 가지 다른 화면 구조로 비교하기 위한 화면입니다. 설정 내용은 같고 이동 방식만 다릅니다.',
  },
  {
    term: '사전 계산 기준',
  ...
  {
    term: '설정 묶음',
    description: '비슷한 설정을 한곳에 모은 영역입니다. 알림 설정, 보안 설정, 화면 설정처럼 나뉩니다.',
  },
  {
    term: '대화상자',
    description: '설정 설명을 확인할 때 잠깐 열리는 작은 창입니다.',
  },
  {
    term: '높은 대비',
    description: '글자와 배경의 차이를 더 크게 보여 주는 화면 보기 설정입니다.',
  },
];
...
function createRunnerState() {
  ...
  const task = settingsTasks[taskIndex] ?? settingsTasks[0];
  runtime.modal = null;
  runtime.isSaving = false;
  runtime.liveStatus = '설정을 바꾼 뒤 해당 묶음 저장을 누르십시오.';
...
function createConditionRuntime(variantId) {
  return {
    variantId,
    currentSectionId: settingsScenario.sections[0]?.id ?? null,
    settingValues: deepClone(settingsScenario.initialValues),
    helpVisitedThisTask: {},
    modal: null,
    liveStatus: '설정을 바꾸면 저장해야 반영됩니다.',
    taskResults: [],
...
function getCurrentTask() {
  if (APP_MODE === 'runner') return settingsTasks[state.taskIndex] ?? null;
  return settingsTasks[state.currentTaskIndex] ?? null;
}
...
function continueAfterTask() {
  if (APP_MODE === 'runner') return;
  if (state.currentTaskIndex < settingsTasks.length - 1) {
...
function launchRunnerTask() {
  ...
  const launchId = uniqueId('settings-launch');
  const payload = {
    launchId,
    sessionId: state.sessionId,
    conditionId,
...
function handleRootClick(event) {
  ...
  if (action === 'save-section') {
    event.preventDefault();
    saveSection(actionTarget.dataset.sectionId, actionTarget.dataset.focusId);
    return;
  }
...
function getSectionById(sectionId) {
  return settingsScenario.sections.find((section) => section.id === sectionId) ?? null;
}

function getSettingById(settingId) {
  return settingsScenario.settings.find((setting) => setting.id === settingId) ?? null;
}

function getSettingsBySection(sectionId) {
  return settingsScenario.settings.filter((setting) => setting.sectionId === sectionId);
}

function getCurrentSection(run) {
  return getSectionById(run.currentSectionId) ?? settingsScenario.sections[0] ?? null;
}
...
function formatRunStateSummary(run) {
  ...
  const lastSavedSection = run.lastSavedSectionId ? getSectionById(run.lastSavedSectionId) : null;
  return [
    currentSection ? `현재 묶음 ${currentSection.label}` : '현재 묶음 없음',
    lastSavedSection ? `최근 저장 ${lastSavedSection.label}` : '최근 저장 없음',
  ].join(' / ');
}
...
function isTaskSatisfied(task, run, sectionId) {
  if (!task || !run) return false;
  if (task.saveSectionId && sectionId !== task.saveSectionId) return false;
...
function noteWrongSettingAction(actionType, payload = {}) {
  ...
  } else if (actionType === 'save-section') {
    isWrong = !isTaskSatisfied(task, run, payload.sectionId);
...
function updateSettingValue(settingId, value, { focusSelector = '', actionType = 'choice' } = {}) {
  ...
  run.liveStatus = `${setting.label}을 ${getSettingValueLabel(setting, value)} 상태로 바꾸었습니다.`;
  if (focusSelector) {
...
function saveSection(sectionId, focusId) {
  const run = getCurrentRun();
  const task = getCurrentTask();
  const section = getSectionById(sectionId);
  if (!run || !task || !section || run.isSaving) return;

  noteWrongSettingAction('save-section', { sectionId });
  run.isSaving = true;
  run.currentSectionId = section.id;
  run.liveStatus = `${section.label}을 저장하는 중입니다…`;
  render();

  window.setTimeout(() => {
    run.isSaving = false;
    run.lastSavedSectionId = section.id;
    run.currentTaskLogger?.note('save-section', {
      sectionId,
      values: deepClone(run.settingValues),
    });

    if (isTaskSatisfied(task, run, section.id)) {
      finishRunnerTask(`saved-${section.id}`);
      return;
    }

    run.liveStatus = `${section.label}을 저장했습니다.`;
    if (state.conditionId === 'variantB' && focusId) {
      requestFocus(`[data-focus-id="${focusId}"]`);
    } else {
      requestFocus(`#section-heading-${section.id}`);
    }
    render();
  }, 260);
}
...
function finishRunnerTask(reason) {
  ...
  const summary = run.currentTaskLogger.finish({
    ...
    notes: [
      ...
      `lastSaved=${run.lastSavedSectionId ?? 'none'}`,
...
function handleSectionTabNavigation(event, currentButton) {
  ...
  const run = getCurrentRun();
  if (!run) return;
  const currentIndex = settingsScenario.sections.findIndex((section) => section.id === currentButton.dataset.sectionId);
  if (currentIndex === -1) return;

  let nextIndex = currentIndex;
  if (event.key === 'ArrowRight' || event.key === 'ArrowDown') nextIndex = Math.min(currentIndex + 1, settingsScenario.sections.length - 1);
  if (event.key === 'ArrowLeft' || event.key === 'ArrowUp') nextIndex = Math.max(currentIndex - 1, 0);
  if (event.key === 'Home') nextIndex = 0;
  if (event.key === 'End') nextIndex = settingsScenario.sections.length - 1;

  event.preventDefault();
  const nextSection = settingsScenario.sections[nextIndex];
...
function getDocumentTitle() {
  if (APP_MODE === 'runner') {
    const task = getCurrentTask();
    return `수행 화면 · ${task?.title ?? '설정 화면'}`;
  }
  if (state.view === 'serviceIntro') return '설정 화면 서비스 화면';
  if (state.view === 'taskPrep' || state.view === 'taskRunning') return `과업 준비 · ${getCurrentTask()?.title ?? '설정 화면'}`;
...
function renderRunnerPage() {
  ...
  return `
    <div class="runner-shell">
      ${conditionId === 'variantB' && !state.completed ? `<a class="skip-link" href="#settings-heading" data-action="jump-results" data-focus-id="runner-skip-settings">${RUNNER_LABELS.quickJump}</a>` : ''}
      <main class="runner-main" aria-label="설정 화면 수행 화면" ${state.completed ? 'inert aria-hidden="true"' : ''}>
...
function renderServiceIntroView() {
  return `
    <header class="hero card">
      ...
      <h1 id="service-heading" tabindex="-1">설정 화면</h1>
      <p>
        같은 계정 설정 내용을 두 가지 다른 이동 구조로 보여 주는 실험 화면입니다.
      ...
      <div class="hero-grid">
        <section>
          <h2>이번에 확인하는 것</h2>
          <ul>
            <li>길게 이어진 설정 목록과 흩어진 설명 보기 버튼이 순차 탐색 부담을 얼마나 키우는지</li>
            <li>설정 묶음을 빠르게 고르고 같은 묶음 안에서 바로 저장할 수 있을 때 부담이 얼마나 줄어드는지</li>
...
function renderTaskPreparationView() {
  ...
  const run = getCurrentRun();
  const task = getCurrentTask();
  const benchmark = benchmarkResultsSettings.variants[conditionId].tasks[task.benchmarkTaskId];
  const activeLaunch = state.activeLaunch;
  ...
  return `
    <section class="card review-hero">
      ...
      <div class="pill-group">
        ...
        <span class="pill">과업 ${state.currentTaskIndex + 1} / ${settingsTasks.length}</span>
      </div>
    </section>
    ...
    <section class="review-grid">
      <article class="card">
        ...
        <dl class="meta-list compact">
          <div><dt>현재 설정 상태</dt><dd>${escapeHtml(formatTaskCurrentState(task, run))}</dd></div>
          <div><dt>목표 설정 묶음</dt><dd>${escapeHtml(targetSection?.label ?? '없음')}</dd></div>
          <div><dt>목표 설정 상태</dt><dd>${escapeHtml(task.targetSummary)}</dd></div>
        </dl>
      </article>
...
function renderTaskReviewView() {
  ...
  const benchmark = benchmarkResultsSettings.variants[conditionId].tasks[result.benchmarkTaskId];
  const comparison = benchmarkResultsSettings.comparisons[result.benchmarkTaskId];
    ...
    <section class="review-grid">
      <article class="card">
        ...
        <dl class="meta-list compact">
          ...
          <div><dt>목표와 다른 설정에서 동작</dt><dd>${result.wrongSelections}</dd></div>
      ...
      <article class="card">
        ...
        <p class="muted">${escapeHtml(benchmarkResultsSettings.variants[conditionId].description)}</p>
      </article>
    </section>
    <section class="card">
      <h2>비교안 간 예상 차이</h2>
      <table class="summary-table">
        ...
        <tbody>
          ${Object.entries(comparison).map(([profileId, value]) => `
            <tr>
              <th>${escapeHtml(benchmarkResultsSettings.overall[profileId].label)}</th>
      ...
      <div class="button-row">
        <button class="button button-primary" data-action="continue-after-task">
          ${state.currentTaskIndex < settingsTasks.length - 1 ? '다음 과업 준비' : '현재 비교안 요약 보기'}
        </button>
        <button class="button button-secondary" data-action="restart-experiment">처음부터 다시 시작</button>
      </div>
    </section>
    ...
    <section class="review-grid">
      <article class="card">
        ...
        <dl class="meta-list compact">
          ...
          <div><dt>목표와 다른 설정에서 동작</dt><dd>${totals.wrongSelections}</dd></div>
      ...
      <article class="card">
        <h2>저장된 과업 기록</h2>
        <ol>
          ${run.taskResults.map((result, index) => `<li><strong>${escapeHtml(settingsTasks[index].title)}</strong> — ${formatSeconds(result.durationSeconds)}, 키 ${result.totalKeyInputs}회, 초점 이동 ${result.focusChanges}회</li>`).join('')}
        </ol>
      </article>
    </section>
    <div class="button-row">
...
function renderFinalView() {
  ...
  return `
    ...
    <section class="card toolbar-card">
      <label>
        ...
        <select name="benchmark-profile">
          ${Object.entries(benchmarkResultsSettings.overall).map(([profileId, profile]) => `
            <option value="${profileId}" ${profileId === selectedProfileId ? 'selected' : ''}>${escapeHtml(profile.label)}</option>
          `).join('')}
        </select>
      </label>
      <div class="button-row">
        <a class="button button-secondary" download="settings-screen-${escapeHtml(state.sessionId)}.json" href="${exportUrl}">결과 파일(JSON) 내려받기</a>
      </div>
    </section>
    ...
    ${renderStudyServiceCompletionCard({ sessionId: state.sessionId, serviceId: 'settings', serviceLabel: '설정 화면' })}
    <section class="card">
      <h2>실제 기록 비교</h2>
      <table class="summary-table">
        <thead>
        ...
        <tbody>
          ...
          <tr><th>목표와 다른 설정에서 동작</th><td>${actualA.wrongSelections}</td><td>${actualB.wrongSelections}</td><td>${formatSigned(actualB.wrongSelections - actualA.wrongSelections)}</td></tr>
    ...
    <section class="card">
      ...
      <ul>
        <li>설정 화면도 메인 창과 수행 탭을 분리해 같은 운영 방식으로 확장했습니다.</li>
  ...
  return `
    <section class="card settings-workspace-card">
      <div class="results-header">
        <div>
          <h2 id="settings-heading" tabindex="-1">설정 항목</h2>
          <p class="muted">${escapeHtml(settingsScenario.pageSummary)}</p>
        </div>
        ${conditionId === 'variantB'
          ? '<p class="keyboard-tip">방향키로 설정 묶음을 고르고, 같은 묶음 안에서 설정을 바꾼 뒤 저장</p>'
          : '<p class="keyboard-tip">탭 키와 Shift+탭 키로 설명 보기, 값 선택, 저장 버튼을 차례대로 이동</p>'}
      </div>
...
function renderVariantASettingsLayout(run) {
  return `
    <div class="settings-section-stack">
      ${settingsScenario.sections.map((section) => renderVariantASection(section, run)).join('')}
    </div>
  `;
}
...
function renderVariantASection(section, run) {
  ...
  return `
    <article class="settings-section-card settings-section-card-a">
      ...
      <div class="button-row">
        <button class="button button-primary" data-action="save-section" data-section-id="${section.id}" data-focus-id="save-section-${section.id}" ${run.isSaving ? 'disabled' : ''}>
          ${run.isSaving && run.currentSectionId === section.id ? `${escapeHtml(section.label)} 저장 중…` : `${escapeHtml(section.label)} 저장`}
        </button>
      </div>
    </article>
  `;
  ...
  return `
    <section class="setting-row setting-row-a">
      ...
      <div class="button-row compact-row">
        <a href="#" class="inline-link" data-focus-id="setting-status-${setting.id}" data-inert-link="true">현재 상태 ${escapeHtml(valueLabel)}</a>
        <a href="#" class="inline-link" data-focus-id="setting-changed-${setting.id}" data-inert-link="true">최근 변경 ${escapeHtml(setting.changedAt)}</a>
  ...
  return `
    <div class="settings-composite-layout">
      ...
      <section class="card selected-settings-card" role="tabpanel" aria-labelledby="selected-section-title">
        <div class="settings-section-head">
          <div>
            <h3 id="selected-section-title">${escapeHtml(section?.label ?? '설정 묶음')}</h3>
            <p class="muted">${escapeHtml(section?.summary ?? '')}</p>
          </div>
        ...
        <div class="settings-item-list settings-item-list-b">
          ${section ? getSettingsBySection(section.id).map((setting) => renderVariantBSetting(setting, run)).join('') : '<p class="muted">표시할 설정 묶음이 없습니다.</p>'}
        </div>
        ${section ? `
          <div class="button-row">
            <button class="button button-primary" data-action="save-section" data-section-id="${section.id}" data-focus-id="save-section-${section.id}" ${run.isSaving ? 'disabled' : ''}>
  ...
  return `
    <div class="modal-backdrop">
      <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="dialog-title" aria-describedby="dialog-description" data-modal-dialog>
        ...
        <dl class="meta-list compact">
          <div><dt>설정 묶음</dt><dd>${escapeHtml(getSectionById(setting.sectionId)?.label ?? '')}</dd></div>
          <div><dt>현재 상태</dt><dd>${escapeHtml(getSettingValueLabel(setting, run.settingValues[setting.id]))}</dd></div>
          <div><dt>최근 변경</dt><dd>${escapeHtml(setting.changedAt)}</dd></div>
        </dl>
