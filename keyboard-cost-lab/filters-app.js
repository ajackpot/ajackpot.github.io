import { filtersScenario } from './data/filters-scenario.js';
import { filtersTasks } from './data/tasks-filters.js';
import { benchmarkResultsFilters } from './data/benchmark-results-filters.js';
...
const LAUNCH_STORAGE_PREFIX = 'keyboard-cost-lab-filters-launch';
const CHANNEL_PREFIX = 'keyboard-cost-lab-filters-channel';
const CHANNEL_FALLBACK_STORAGE_PREFIX = 'keyboard-cost-lab-filters-channel-fallback';
const SURVEY_CONFIG = {
  baseUrl: '',
};
...
const VARIANT_META = {
  variantA: {
    ...
    subtitle: '상단 보조 링크와 세부 조건 선택을 길게 지나야 검색 결과에 도달하고, 조건 선택지와 자료 작업이 여러 개로 흩어져 있어 순차 이동이 길며, 미리보기 대화상자를 닫으면 검색 결과 제목 근처부터 다시 찾아야 하는 구조',
    improvements: [
      '상단 보조 링크와 세부 조건 설명 링크를 지난 뒤 기간, 자료 종류, 담당 부서, 대상, 첨부 조건을 맞춰야 합니다.',
      '비교안 A에서는 조건 선택지가 여러 개의 개별 버튼으로 흩어져 있어 순차 이동이 길어집니다.',
  ...
  variantB: {
    shortLabel: 'B',
    title: '비교안 B · 개선 구조',
    subtitle: '세부 조건으로 바로 이동하고, 적은 탭 수로 조건을 맞춘 뒤, 자료를 하나의 선택 항목으로 고르고, 선택한 자료 작업을 한곳에서 이어서 수행하는 구조',
    improvements: [
      '세부 조건으로 바로 이동해 첫 진입 부담을 줄입니다.',
      '비교안 B에서는 기간, 자료 종류, 담당 부서, 대상, 첨부 여부를 적은 탭 수로 설정합니다.',
...
const RUNNER_LABELS = {
  quickJump: '세부 조건으로 바로 이동',
  footerJump: '세부 조건으로 이동',
};
...
const GLOSSARY_ENTRIES = [
  ...
  {
    term: '세부 조건',
    description: '기간, 자료 종류, 담당 부서, 대상처럼 원하는 자료를 좁혀 찾기 위해 먼저 고르는 조건입니다.',
  },
  {
    term: '첨부 있는 자료만 보기',
    description: '파일이 함께 붙은 자료만 따로 보이게 하는 조건입니다.',
  },
  {
    term: '미리보기',
...
function createRunnerState() {
  ...
  const task = filtersTasks[taskIndex] ?? filtersTasks[0];
  runtime.modal = null;
  runtime.isApplying = false;
  runtime.isWorking = false;
  runtime.liveStatus = '세부 조건을 맞춘 뒤 원하는 자료를 찾으십시오.';
...
function createConditionRuntime(variantId) {
  return {
    ...
    attachmentOnlyDraft: false,
    savedByResultId: {},
    openedResultId: null,
    currentResultId: filtersScenario.results[0]?.id ?? null,
    previewVisitedThisTask: {},
    modal: null,
    liveStatus: '세부 조건을 바꾸면 검색 결과 목록이 갱신됩니다.',
    taskResults: [],
...
function getCurrentTask() {
  if (APP_MODE === 'runner') return filtersTasks[state.taskIndex] ?? null;
  return filtersTasks[state.currentTaskIndex] ?? null;
}

function hydrateConditionRuntime(variantId, snapshot = {}) {
  const runtime = createConditionRuntime(variantId);
  runtime.period = snapshot.period ?? runtime.period;
  runtime.periodDraft = snapshot.periodDraft ?? runtime.periodDraft;
  runtime.type = snapshot.type ?? runtime.type;
  runtime.typeDraft = snapshot.typeDraft ?? runtime.typeDraft;
  runtime.department = snapshot.department ?? runtime.department;
  runtime.departmentDraft = snapshot.departmentDraft ?? runtime.departmentDraft;
  runtime.audience = snapshot.audience ?? runtime.audience;
  runtime.audienceDraft = snapshot.audienceDraft ?? runtime.audienceDraft;
  runtime.attachmentOnly = snapshot.attachmentOnly ?? runtime.attachmentOnly;
  runtime.attachmentOnlyDraft = snapshot.attachmentOnlyDraft ?? runtime.attachmentOnlyDraft;
...
function serializeRuntimeSnapshot(run) {
  return {
    ...
    period: run.period,
    periodDraft: run.periodDraft,
    type: run.type,
    typeDraft: run.typeDraft,
    department: run.department,
    departmentDraft: run.departmentDraft,
    audience: run.audience,
    audienceDraft: run.audienceDraft,
    attachmentOnly: run.attachmentOnly,
    attachmentOnlyDraft: run.attachmentOnlyDraft,
...
function applyRuntimeSnapshot(targetRun, snapshot) {
  ...
  targetRun.period = hydrated.period;
  targetRun.periodDraft = hydrated.periodDraft;
  targetRun.type = hydrated.type;
  targetRun.typeDraft = hydrated.typeDraft;
  targetRun.department = hydrated.department;
  targetRun.departmentDraft = hydrated.departmentDraft;
  targetRun.audience = hydrated.audience;
  targetRun.audienceDraft = hydrated.audienceDraft;
  targetRun.attachmentOnly = hydrated.attachmentOnly;
  targetRun.attachmentOnlyDraft = hydrated.attachmentOnlyDraft;
...
function continueAfterTask() {
  if (APP_MODE === 'runner') return;
  if (state.currentTaskIndex < filtersTasks.length - 1) {
...
function handleRootClick(event) {
  ...
  if (action === 'select-filter-option') {
    event.preventDefault();
    const run = getCurrentRun();
    if (!run) return;
    const group = actionTarget.dataset.filterGroup;
    const value = actionTarget.dataset.filterValue ?? '';
    if (group === 'period') run.periodDraft = value;
    if (group === 'type') run.typeDraft = value;
    if (group === 'department') run.departmentDraft = value;
    if (group === 'audience') run.audienceDraft = value;
    if (group === 'attachment') run.attachmentOnlyDraft = value === 'true';
    render();
    return;
  }
  ...
  if (action === 'jump-results') {
    event.preventDefault();
    focusElementNow('#filter-heading');
    return;
  }
...
function handleRootChange(event) {
  ...
  const run = getCurrentRun();
  if (!run) return;
  if (element.name === 'period') run.periodDraft = element.value;
  if (element.name === 'type') run.typeDraft = element.value;
  if (element.name === 'department') run.departmentDraft = element.value;
  if (element.name === 'audience') run.audienceDraft = element.value;
  if (element.name === 'attachment-only') run.attachmentOnlyDraft = element.checked;
}
...
function getResultById(resultId) {
  return filtersScenario.results.find((result) => result.id === resultId) ?? null;
}

function getPeriodLabel(periodId) {
  return filtersScenario.periodOptions.find((option) => option.id === periodId)?.label ?? periodId;
}


function getTypeLabel(typeId) {
  return filtersScenario.typeOptions.find((option) => option.id === typeId)?.label ?? typeId;
}

function getDepartmentLabel(departmentId) {
  return filtersScenario.departmentOptions.find((option) => option.id === departmentId)?.label ?? departmentId;
}

function getAudienceLabel(audienceId) {
  return filtersScenario.audienceOptions.find((option) => option.id === audienceId)?.label ?? audienceId;
}

function getAttachmentLabel(attachmentOnly) {
  return attachmentOnly ? '첨부 있는 자료만' : '첨부 조건 없음';
}

function isWithinPeriod(result, periodId) {
  if (periodId === 'all') return true;
  const now = new Date('2026-03-31T23:59:59');
  const updatedAt = new Date(result.updatedAt);
  const diffDays = (now.getTime() - updatedAt.getTime()) / (1000 * 60 * 60 * 24);
  if (periodId === 'last7') return diffDays <= 7;
  if (periodId === 'last30') return diffDays <= 30;
  if (periodId === 'last90') return diffDays <= 90;
  return true;
}
...
function getVisibleSearch(run) {
  const filtered = filtersScenario.results.filter((result) => {
    if (!isWithinPeriod(result, run.period)) return false;
    if (run.type !== 'all' && result.type !== run.type) return false;
    if (run.department !== 'all' && result.department !== run.department) return false;
    if (run.audience !== 'all' && result.audience !== run.audience && result.audience !== 'all') return false;
    if (run.attachmentOnly && !result.hasAttachment) return false;
    return true;
  });

  return filtered.slice().sort((left, right) => right.updatedAt.localeCompare(left.updatedAt));
}
...
function formatResultLabel(result, run) {
  return `${result.title} · ${result.badge} · ${getDepartmentLabel(result.department)} · ${result.hasAttachment ? '첨부 있음' : '첨부 없음'} · ${result.timeLabel} · 저장 ${getEffectiveSaveCount(result, run)}`;
}
...
function formatRunStateSummary(run) {
  ...
  return [
    `기간 ${getPeriodLabel(run.period)}`,
    `자료 종류 ${getTypeLabel(run.type)}`,
    `담당 부서 ${getDepartmentLabel(run.department)}`,
    `대상 ${getAudienceLabel(run.audience)}`,
    getAttachmentLabel(run.attachmentOnly),
...
function applyResultFilters() {
  ...
  run.isApplying = true;
  run.liveStatus = '세부 조건을 적용하는 중입니다…';
  render();

  window.setTimeout(() => {
    run.isApplying = false;
    run.period = run.periodDraft;
    run.type = run.typeDraft;
    run.department = run.departmentDraft;
    run.audience = run.audienceDraft;
    run.attachmentOnly = run.attachmentOnlyDraft;
    ensureCurrentResultVisible(run);
    const visibleSearch = getVisibleSearch(run);
...
function isTaskSatisfied(task, run) {
  ...
  if (task.requiredPeriod && run.period !== task.requiredPeriod) return false;
  if (task.requiredType && run.type !== task.requiredType) return false;
  if (task.requiredDepartment && run.department !== task.requiredDepartment) return false;
  if (task.requiredAudience && run.audience !== task.requiredAudience) return false;
  if (typeof task.requiredAttachmentOnly === 'boolean' && run.attachmentOnly !== task.requiredAttachmentOnly) return false;
...
function getDocumentTitle() {
  if (APP_MODE === 'runner') {
    const task = getCurrentTask();
    return `수행 화면 · ${task?.title ?? '검색 세부 조건'}`;
  }
  if (state.view === 'serviceIntro') return '검색 세부 조건 서비스 화면';
  if (state.view === 'taskPrep' || state.view === 'taskRunning') return `과업 준비 · ${getCurrentTask()?.title ?? '검색 세부 조건'}`;
  if (state.view === 'taskReview') return '과업 결과 요약';
  if (state.view === 'conditionReview') return '비교안 요약';
  return '검색 세부 조건 최종 비교';
}
...
function renderRunnerPage() {
  ...
  return `
    <div class="runner-shell">
      ${conditionId === 'variantB' && !state.completed ? `<a class="skip-link" href="#filter-heading" data-action="jump-results" data-focus-id="runner-skip-filters">${RUNNER_LABELS.quickJump}</a>` : ''}
      <main class="runner-main" aria-label="검색 세부 조건 수행 화면" ${state.completed ? 'inert aria-hidden="true"' : ''}>
...
function renderServiceIntroView() {
  return `
    <header class="hero card">
      ...
      <h1 id="service-heading" tabindex="-1">검색 세부 조건</h1>
      <p>
        같은 자료 목록을 두 가지 다른 세부 조건 구조로 보여 주는 실험 화면입니다.
      ...
      <div class="hero-grid">
        <section>
          <h2>이번에 확인하는 것</h2>
          <ul>
            <li>기간, 자료 종류, 담당 부서, 대상, 첨부 조건이 흩어져 있을 때 순차 탐색 부담이 얼마나 커지는지</li>
            <li>세부 조건을 적은 탭 수로 묶고, 결과 작업을 한곳에 모았을 때 부담이 얼마나 줄어드는지</li>
...
function renderTaskPreparationView() {
  ...
  const run = getCurrentRun();
  const task = getCurrentTask();
  const benchmark = benchmarkResultsFilters.variants[conditionId].tasks[task.benchmarkTaskId];
  const activeLaunch = state.activeLaunch;
  ...
  return `
    <section class="card review-hero">
      ...
      <div class="pill-group">
        ...
        <span class="pill">과업 ${state.currentTaskIndex + 1} / ${filtersTasks.length}</span>
      </div>
    </section>
    ...
    <section class="review-grid">
      <article class="card">
        ...
        <dl class="meta-list compact">
          <div><dt>현재 세부 조건</dt><dd>${escapeHtml(formatRunStateSummary(run))}</dd></div>
...
function renderTaskReviewView() {
  ...
  const benchmark = benchmarkResultsFilters.variants[conditionId].tasks[result.benchmarkTaskId];
  const comparison = benchmarkResultsFilters.comparisons[result.benchmarkTaskId];
  ...
  return `
        ...
        <div class="benchmark-delta">
          ...
          <ul>
            ${Object.entries(comparison).map(([profileId, value]) => `
              <li><strong>${escapeHtml(benchmarkResultsFilters.overall[profileId].label)}</strong>: ${value.expectedReductionSeconds}초 감소 예상 (${value.expectedReductionPercent}%)</li>
            `).join('')}
          </ul>
        </div>
      </article>
    ...
    <div class="button-row">
      <button class="button button-primary" data-action="continue-after-task">
        ${state.currentTaskIndex < filtersTasks.length - 1 ? '다음 과업 준비' : '현재 비교안 요약 보기'}
      </button>
      <button class="button button-secondary" data-action="restart-experiment">처음부터 다시 시작</button>
    </div>
  `;
  ...
  return `
    ...
    <section class="review-grid">
      ...
      <article class="card">
        <h2>저장된 과업 기록</h2>
        <ol>
          ${run.taskResults.map((result, index) => `<li><strong>${escapeHtml(filtersTasks[index].title)}</strong> — ${formatSeconds(result.durationSeconds)}, 키 ${result.totalKeyInputs}회, 초점 이동 ${result.focusChanges}회</li>`).join('')}
        </ol>
      </article>
    </section>
    <div class="button-row">
...
function renderFinalView() {
  ...
  return `
    <section class="card review-hero">
      <p class="eyebrow">검색 세부 조건 실험 완료</p>
    ...
    <section class="card toolbar-card">
      <label>
        ...
        <select name="benchmark-profile">
          ${Object.entries(benchmarkResultsFilters.overall).map(([profileId, profile]) => `
            <option value="${profileId}" ${profileId === selectedProfileId ? 'selected' : ''}>${escapeHtml(profile.label)}</option>
          `).join('')}
        </select>
      </label>
      <div class="button-row">
        <a class="button button-secondary" download="search-filters-${escapeHtml(state.sessionId)}.json" href="${exportUrl}">결과 파일(JSON) 내려받기</a>
      </div>
    </section>
    ...
    ${renderStudyServiceCompletionCard({ sessionId: state.sessionId, serviceId: 'filters', serviceLabel: '검색 세부 조건' })}
    <section class="card">
      <h2>실제 기록 비교</h2>
      <table class="summary-table">
        <thead>
    ...
    <section class="card">
      ...
      <ul>
        <li>세부 조건이 많은 검색 화면도 메인 창과 수행 탭을 분리해 같은 운영 방식으로 확장했습니다.</li>
...
function renderResultControls(conditionId, run) {
  const renderChoiceButtons = (label, group, options, selectedValue, focusPrefix) => `
    <section class="card filter-group-card">
      <h3>${escapeHtml(label)}</h3>
      <div class="button-row filter-button-list" role="group" aria-label="${escapeHtml(label)}">
        ${options.map((option) => `
          <button
            class="button ${selectedValue === option.id ? 'button-primary' : 'button-secondary'}"
            data-action="select-filter-option"
            data-filter-group="${group}"
            data-filter-value="${option.id}"
            data-focus-id="${focusPrefix}-${option.id}"
          >
  ...
  if (conditionId === 'variantA') {
    const attachmentOptions = [
      { id: 'false', label: '전체 자료' },
      { id: 'true', label: '첨부 있는 자료만' },
    ];

    return `
      <section class="card filters-card filters-a">
        <div class="filters-header">
          <div>
            <h2 id="filter-heading" tabindex="-1">세부 조건 선택</h2>
            <p class="muted">기간, 자료 종류, 담당 부서, 대상, 첨부 조건을 차례로 맞춘 뒤 조건 적용을 누르십시오.</p>
          </div>
        </div>
        <div class="button-row compact-row">
          <a href="#" class="inline-link" data-focus-id="filter-guide-link-1" data-inert-link="true">기간 조건 설명 보기</a>
          <a href="#" class="inline-link" data-focus-id="filter-guide-link-2" data-inert-link="true">자료 종류 설명 보기</a>
          <a href="#" class="inline-link" data-focus-id="filter-guide-link-3" data-inert-link="true">담당 부서 설명 보기</a>
        </div>
        <div class="filters-stack">
          ${renderChoiceButtons('기간', 'period', filtersScenario.periodOptions, run.periodDraft, 'period-draft')}
          ${renderChoiceButtons('자료 종류', 'type', filtersScenario.typeOptions, run.typeDraft, 'type-draft')}
          ${renderChoiceButtons('담당 부서', 'department', filtersScenario.departmentOptions, run.departmentDraft, 'department-draft')}
          ${renderChoiceButtons('대상', 'audience', filtersScenario.audienceOptions, run.audienceDraft, 'audience-draft')}
          ${renderChoiceButtons('첨부 조건', 'attachment', attachmentOptions, String(run.attachmentOnlyDraft), 'attachment-draft')}
        </div>
        <div class="button-row">
          <a href="#" class="inline-link" data-focus-id="result-policy-link" data-inert-link="true">세부 조건 도움말 보기</a>
          <a href="#" class="inline-link" data-focus-id="result-help-link" data-inert-link="true">첨부 조건 도움말 보기</a>
  ...
  return `
    <section class="card filters-card filters-b">
      <div class="filters-header">
        <div>
          <h2 id="filter-heading" tabindex="-1">세부 조건 선택</h2>
          <p class="muted">적은 탭 수로 조건을 맞춘 뒤 바로 검색 결과를 확인하십시오.</p>
        </div>
      </div>
      <div class="filters-grid">
        <label>
          <span>기간</span>
          <select name="period" data-focus-id="result-period">
            ${filtersScenario.periodOptions.map((option) => `<option value="${option.id}" ${run.periodDraft === option.id ? 'selected' : ''}>${escapeHtml(option.label)}</option>`).join('')}
          </select>
        </label>
        <label>
          <span>자료 종류</span>
          <select name="type" data-focus-id="result-type">
            ${filtersScenario.typeOptions.map((option) => `<option value="${option.id}" ${run.typeDraft === option.id ? 'selected' : ''}>${escapeHtml(option.label)}</option>`).join('')}
          </select>
        </label>
        <label>
          <span>담당 부서</span>
          <select name="department" data-focus-id="result-department">
            ${filtersScenario.departmentOptions.map((option) => `<option value="${option.id}" ${run.departmentDraft === option.id ? 'selected' : ''}>${escapeHtml(option.label)}</option>`).join('')}
          </select>
        </label>
        <label>
          <span>대상</span>
          <select name="audience" data-focus-id="result-audience">
            ${filtersScenario.audienceOptions.map((option) => `<option value="${option.id}" ${run.audienceDraft === option.id ? 'selected' : ''}>${escapeHtml(option.label)}</option>`).join('')}
          </select>
        </label>
        <label class="checkbox-filter">
          <span>첨부 있는 자료만 보기</span>
          <input type="checkbox" name="attachment-only" data-focus-id="result-attachment" ${run.attachmentOnlyDraft ? 'checked' : ''} />
        </label>
      </div>
      <div class="button-row">
        <a href="#" class="inline-link" data-focus-id="result-policy-link" data-inert-link="true">세부 조건 도움말 보기</a>
    ...
    <section class="card results-card">
      <div class="results-header">
        <div>
          ...
          <p class="muted">검색어 ${escapeHtml(filtersScenario.queryLabel)} · 표시된 자료 ${visibleSearch.length}개 · ${escapeHtml(formatRunStateSummary(run))}</p>
        </div>
    ...
    <ul class="result-list result-list-a">
      ${visibleSearch.map((result) => `
        <li class="result-card ${run.openedResultId === result.id ? 'result-card-expanded' : ''}">
          ...
          <p class="muted">담당 부서 ${escapeHtml(getDepartmentLabel(result.department))} · 대상 ${escapeHtml(getAudienceLabel(result.audience))} · ${result.hasAttachment ? '첨부 있음' : '첨부 없음'}</p>
    ...
    <div class="result-composite-layout">
      <div role="listbox" aria-label="검색 결과 목록" class="result-option-list">
        ${visibleSearch.map((result) => `
          ...
          >
            ...
            <span class="muted">${escapeHtml(result.timeLabel)} · ${escapeHtml(getDepartmentLabel(result.department))}</span>
            <span>${escapeHtml(result.summary)}</span>
            <span class="muted">${result.hasAttachment ? '첨부 있음' : '첨부 없음'} · 저장 ${getEffectiveSaveCount(result, run)}</span>
          </button>
        `).join('')}
      </div>
      <section class="card selected-result-card">
        ...
        ${selectedResult ? `
          ...
          <p class="muted">담당 부서 ${escapeHtml(getDepartmentLabel(selectedResult.department))} · 대상 ${escapeHtml(getAudienceLabel(selectedResult.audience))} · ${selectedResult.hasAttachment ? '첨부 있음' : '첨부 없음'}</p>
...
function renderOpenedResultStatus(result) {
  return `
    <section class="result-status-card" aria-label="${escapeHtml(result.title)} 자료 상태">
      ...
      <p class="muted">실험용 화면에서는 실제 새 자료 화면으로 이동하지 않고, 이 자료를 열었다는 상태만 표시합니다.</p>
    </section>
  `;
}
    ...
    <div class="modal-backdrop">
      <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="dialog-title" aria-describedby="dialog-description" data-modal-dialog>
        ...
        <dl class="meta-list compact">
          ...
          <div><dt>담당 부서</dt><dd>${escapeHtml(getDepartmentLabel(result.department))}</dd></div>
          <div><dt>대상</dt><dd>${escapeHtml(getAudienceLabel(result.audience))}</dd></div>
          <div><dt>첨부 여부</dt><dd>${result.hasAttachment ? '첨부 있음' : '첨부 없음'}</dd></div>
