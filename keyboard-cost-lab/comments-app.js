...
function createRunnerState() {
  ...
  runtime.liveStatus = '정렬 기준과 댓글 종류를 정한 뒤 원하는 댓글을 찾아보세요.';
...
function handleRootClick(event) {
  ...
  if (action === 'site-placeholder') {
    event.preventDefault();
    showSiteNotice(actionTarget.dataset.notice || '이 기능은 현재 준비 중입니다. 다른 기능은 이 화면에서 계속 이용할 수 있습니다.');
    return;
  }
  ...
  if (action === 'community-message') {
    ...
    const run = getCurrentRun();
    if (run) {
      setRunSiteNotice(run, actionTarget.dataset.notice || '요청한 작업을 마쳤습니다.');
...
function renderPseudoCommentCombo({ fieldName, label, options, selected }) {
  ...
  return `
    <div class="pseudo-combo pseudo-combo-a" role="group" aria-label="${escapeHtml(label)}">
      ...
      <button class="button button-secondary pseudo-combo-selected" type="button" data-action="community-message" data-notice="아래에서 항목을 선택해 주세요." data-focus-id="comment-${escapeHtml(fieldName)}-selected">
...
function closeModal() {
  ...
  if (closingModal.kind === 'task-final') {
    ...
    run.liveStatus = '댓글 작업을 마쳤습니다. 과업을 마쳤다고 판단하면 맨 아래의 ‘과업 종료’를 누르세요.';
    run.currentTaskLogger?.setModalState({
      open: false,
      containerSelector: null,
      triggerFocusId: closingModal.triggerFocusId,
...
function getEndTaskOutcome(task, run) {
  ...
  let message = '요청한 댓글 작업을 끝내기 전에 과업을 종료했습니다.';
  if (task.completion === 'replyQuestion') {
    const targetAnswer = run.submittedReplyAnswers[task.targetCommentId];
    const otherAnswered = Object.keys(run.submittedReplyAnswers).some((commentId) => commentId !== task.targetCommentId);
    if (otherAnswered) {
      message = '요청한 댓글 대신 다른 댓글의 답글 질문에 답했습니다.';
    ...
    if (helpfulIds.some((commentId) => commentId !== task.targetCommentId)) {
      message = '요청한 댓글 대신 다른 댓글에 ‘도움이 돼요’를 표시했습니다.';
      ...
      message = '요청한 댓글의 정보를 확인한 기록이 없습니다.';
    }
  }
...
function renderServiceIntroView() {
  return `
    <header class="hero card">
      ...
      <p>댓글 목록에서 원하는 글을 찾고 답글이나 댓글 정보를 확인합니다. 과업은 새 탭에서 진행합니다.</p>
      <div class="hero-grid">
        <section>
          <h2>진행 순서</h2>
          <ol>
            ...
            <li>과업을 마쳤다고 판단하면 수행 페이지 아래쪽의 ‘과업 종료’를 누릅니다.</li>
          </ol>
        </section>
        <section>
          <h2>알아두세요</h2>
          <ul>
            ...
            <li>과업을 완료하기 어렵다면 현재 상태를 저장하고 다음 단계로 넘어갈 수 있습니다.</li>
  ...
  return `
    <section class="card review-hero">
      <div>
        ...
        <p>아래 요청을 확인한 뒤 댓글 목록 과업 수행 페이지를 열어 주세요.</p>
      </div>
  ...
  return `
    ...
    <div class="button-row">
      <button class="button button-primary" data-action="continue-after-task">
        ${state.currentTaskIndex < commentsTasks.length - 1 ? '다음 과업 준비하기' : '현재 비교안 요약 보기'}
      </button>
      <button class="button button-secondary" data-action="restart-experiment">처음부터 다시 시작</button>
    </div>
  `;
