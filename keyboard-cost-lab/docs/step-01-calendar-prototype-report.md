# 1단계 보고서 · 예약 캘린더 A/B 시범 구현 + 사전 계산
...
1. 정적 웹앱 수준의 A/B 비교 페이지
2. 3개 과업에 대한 사전 사전 계산 기준 스크립트
3. 실제 수행 로그와 사전 사전 계산 기준를 한 화면에서 비교하는 결과 요약
...
- 필터 적용 → 결과 탐색 → 슬롯 대화상자 → 예약/변경/취소
- 과업 직후 실제 로그와 사전 사전 계산 기준 비교
- 조건 종료 후 요약
- 최종 A/B 비교 + 결과 파일(JSON) 내려받기 + 설문 링크용 URL 생성
...
- 슬롯 행마다 `선택` / `상세` 버튼 분리
- 대화상자 열림 시 초기 초점 이동 없음
- 대화상자 닫힘 시 결과 상단으로 복귀
- 현재 예약 패널이 결과 뒤에 위치

### 조건 B · 개선 구조

- 결과로 이동 가능한 결과로 바로 이동 링크 제공
- 슬롯을 한 번만 진입하는 시간표처럼 묶어 단일 탭 진입 후 방향키 이동
- 대화상자 열림 시 첫 액션으로 초점 이동
- 대화상자 닫힘 시 호출한 슬롯으로 복귀
- 현재 예약 패널을 더 빠른 위치에 배치
- 필터 적용 후 결과 제목으로 초점 이동

## 4. 이번 단계의 사전 계산 기준 결과
...
- 사전 계산 기준 생성 스크립트를 실제 실행해 결과 파일을 생성했다.

## 8. 아직 남아 있는 점

### 8.1 아직 거친 부분

- 실제 사용자 검증 전이므로 사전 계산 기준 파라미터는 교정 전 값이다.
...
- 사전 계산 기준는 예측 도구이며, 실제 로그로 교정해야 한다.
