...
for (const [file, source] of Object.entries(sources)) {
  ...
  assert(source.includes('renderTaskRequestVisibilitySwitchHtml({ checked: state.runnerTaskRequestVisible })'), `${file}: 수행 탭 과업 요청 표시 스위치가 없습니다.`);
  assert(source.includes('renderRunnerTaskRequestHtml({ goalSummary: task.goalSummary })'), `${file}: 수행 탭 과업 요청 표시 영역이 없습니다.`);
  assert(!source.includes('${escapeHtml(task.title)} ·'), `${file}: 수행 탭 숨김 제목에 과업명이 남아 있습니다.`);
}

assert(utils.includes('과업 요청 사항을 수행 탭에서도 보기'), 'lib/utils.js: 요청한 스위치 이름이 없습니다.');
