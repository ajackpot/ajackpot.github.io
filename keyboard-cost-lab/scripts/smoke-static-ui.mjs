...
for (const file of appFiles) {
  ...
  assert(source.includes('runnerTaskRequestVisible: Boolean(state.runnerTaskRequestVisible)'), `${file}: 수행 탭 과업 요청 표시 옵션을 시작 정보에 저장함`);
  assert(source.includes('showTaskRequestInRunner: Boolean(launchPayload.runnerTaskRequestVisible)'), `${file}: 수행 탭이 과업 요청 표시 옵션을 읽음`);
  assert(!source.includes('${escapeHtml(task.title)} ·'), `${file}: 수행 탭 숨김 제목에 과업명이 노출되지 않음`);
}
...
assert(utilsSource.includes('과업 요청 사항을 수행 탭에서도 보기'), '공통 유틸에 요청 사항 표시 스위치 문구가 있음');
