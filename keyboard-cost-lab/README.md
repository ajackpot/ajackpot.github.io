...
- 예약 캘린더 첫 과업은 상담 주제 선택, 사전 작성란 비워 두기, 문자 알림 선택을 거쳐 예약하는 흐름입니다. 두 번째 과업은 기존 예약 취소 뒤 새 예약을 진행하는 흐름입니다. 한 주 최대 예약 2개 제한도 함께 반영했습니다.
...
- `검색 결과 목록`: 정렬 기준과 자료 범위를 바꾼 뒤 미리보기 내용에 답하거나, 자료를 저장 옵션과 함께 보관하는 과업. 저장과 바로 열기는 토스트 알림으로 처리하고, 미리보기 닫기는 원래 흐름으로 돌아옵니다. 자료실 홈, 검색 도움말, 최근 검색, 보관함, 자료 요청, 이용 안내, 문의, 검색 알림 같은 보조 기능은 화면 안 패널과 상태 변경으로 반응합니다.
...
이전 단계 보고서는 구현 이력을 남기기 위해 `docs` 폴더에 계속 보관합니다. 다만 현재 공개 범위와 운영 규칙은 이 README, 수동 점검표, 23단계 보고서를 기준으로 확인합니다.
