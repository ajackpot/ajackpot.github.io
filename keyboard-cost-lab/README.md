...
- `댓글 목록`: 반복되는 댓글 카드에서 답글 목록 확인, 댓글 정보 확인, 반응 남기기를 수행하는 과업. 답글 내용은 고정하되 답글 작성자 이름과 정답 선택지는 수행 화면마다 무작위로 배정합니다.
- `검색 결과 목록`: 검색 결과와 반복 자료 카드에서 미리보기, 저장, 바로 열기를 수행하는 과업. 저장과 바로 열기는 토스트 알림으로 처리하고, 미리보기 닫기는 원래 흐름으로 돌아옵니다. 자료실 홈, 검색 도움말, 최근 검색, 보관함, 자료 요청, 이용 안내, 문의, 검색 알림 같은 보조 기능은 화면 안 패널과 상태 변경으로 반응합니다.
...
- 검색 결과 목록의 간단한 보조 기능은 화면 안 패널, 토스트, 상태 변경으로 반응합니다.
