...
- 예약 캘린더 과업은 상담 옵션 선택이 필요한 예약과, 기존 예약 취소 뒤 새 예약을 진행하는 흐름으로 정리했습니다. 한 주 최대 예약 2개 제한도 함께 반영했습니다.
...
- `예약 캘린더`: 날짜·상담사·상담 방식 조건을 맞추고, 일부 예약에서 상담 주제·사전 작성·알림 방법을 선택하거나 기존 예약을 취소한 뒤 새 예약을 진행하는 과업
- `댓글 목록`: 조건을 바꿔 댓글을 좁힌 뒤 답글 작성자를 확인하거나, 댓글 정보 보기를 열고 닫은 뒤 도움이 돼요를 남기는 과업. 답글 내용은 고정하되 답글 작성자 이름과 정답 선택지는 수행 화면마다 무작위로 배정합니다.
...
이전 단계 보고서는 구현 이력을 남기기 위해 `docs` 폴더에 계속 보관합니다. 다만 현재 공개 범위와 운영 규칙은 이 README, 수동 점검표, 22단계 보고서를 기준으로 확인합니다.
