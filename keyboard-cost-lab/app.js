...
function hasRequiredBookingOptions(task, run, slotId) {
  ...
  return Boolean(selected)
    && Object.entries(expected).every(([key, value]) => String(selected[key] ?? '').trim() === String(value ?? '').trim());
}
...
function renderSimulatedHeader(conditionId, run) {
  ...
  const navAttrs = conditionId === 'variantB' && navCollapsed
    ...
    : '';
  const navLinksHtml = conditionId === 'variantB' && navCollapsed
    ? ''
...
function renderIntakeNoteField({ selected }) {
  return `
    <label class="booking-note-field">
      <span>사전 작성란</span>
      <textarea
        name="booking-option-intakeNote"
        rows="4"
        data-focus-id="booking-option-intake-note"
        placeholder="상담 전에 전달할 내용을 적을 수 있습니다."
    ...
    <div class="modal-backdrop">
      <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="dialog-title" aria-describedby="dialog-description" data-modal-dialog>
        ...
        <div class="filters-grid booking-options-grid ${state.conditionId === 'variantA' ? 'booking-options-a' : 'booking-options-b'}">
          ...
          ${renderIntakeNoteField({ selected: run.bookingOptionDraft.intakeNote })}
        </div>
        <div class="button-row">
