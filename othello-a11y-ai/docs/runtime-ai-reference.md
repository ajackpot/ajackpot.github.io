...
| `js/ai/search-engine.js` | 탐색 엔진 본체 | opening → classic search / MCTS search → exact/WLD lane 통합, direct 기본 경로도 active MPC profile 상속, classic 내부 노드는 prepared search move path 기본 사용 |
| `js/ai/worker.js` | 워커용 search wrapper | explicit `mpcProfile`가 없을 때 active MPC semantics 유지 |
| `js/ui/engine-client.js` | 워커 우선 + main-thread fallback | UI와 direct engine의 MPC 기본 의미론을 동일하게 유지 |
...
### 2. 일반 탐색 단계
일반 경로의 기본값은 여전히 iterative deepening 기반 alpha-beta / PVS입니다. 다만 설정 패널의 **AI 모드**에서 preset-aware MCTS lane을 고를 수 있습니다.

- `beginner`: `classic / mcts-lite / mcts-guided`
- `easy` 이상: `classic / mcts-guided / mcts-hybrid`
