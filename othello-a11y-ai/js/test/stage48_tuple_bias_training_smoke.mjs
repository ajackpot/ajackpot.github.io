...
const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'othello-stage48-bias-train-'));
...
try {
  const corpusPath = path.join(tempDir, 'tuple-bias-train-smoke.jsonl');
  const evaluationJsonPath = path.join(tempDir, 'zero-eval.json');
  const outputJsonPath = path.join(tempDir, 'trained-tuple-bias.json');
  ...
  await execFileAsync(process.execPath, [
    ...
    '--epochs', '3',
    '--learning-rate', '0',
    '--bias-learning-rate', '4',
    '--l2', '0',
    '--gradient-clip', '6000',
    '--min-visits', '1',
    '--progress-every', '0',
    '--name', 'stage48-trained-bias-smoke',
  ...
  const trainedBucket = trainedProfile?.trainedBuckets?.[0] ?? null;
  assert.equal(trainedProfile.name, 'stage48-trained-bias-smoke');
  assert.ok(Number.isFinite(trainedBucket?.bias));
  assert.ok(Math.abs(trainedBucket.bias - 1200) < 200, `expected learned bias near 1200, got ${trainedBucket?.bias}`);
  assert.ok(Number.isFinite(holdout?.delta?.mae) && holdout.delta.mae < 0, `expected negative MAE delta, got ${holdout?.delta?.mae}`);
  assert.ok(Math.abs(holdout?.candidate?.meanResidual ?? 0) < Math.abs(holdout?.base?.meanResidual ?? 1));
  assert.ok((trainedProfile?.diagnostics?.weightStatsByBucket?.[0]?.biasVisitCount ?? 0) > 0);

  console.log('stage48 tuple bias training smoke passed');
} finally {
