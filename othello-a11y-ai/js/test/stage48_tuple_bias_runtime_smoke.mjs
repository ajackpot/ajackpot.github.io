...
const state = generateRandomState(18, 48_018);
const tupleProfile = makeTupleResidualTrainingProfileFromWeights({
  name: 'stage48-runtime-bias-tuple',
  description: 'stage48 tuple bias runtime smoke profile',
...
assert.equal(tupleCurrent, 333);
assert.equal(tupleOpponent, -333);
...
assert.equal(explanation.tupleResidualSideToMoveBiasContribution, 333);
assert.equal(explanation.tupleResidualEntries.reduce((sum, entry) => sum + entry.value, 0), 0);
assert.equal(explanation.tupleResidualBucketKey, 'late-a');

console.log('stage48 tuple bias runtime smoke passed');
