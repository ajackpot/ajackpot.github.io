...
async def main() -> None:
    ...
    async with async_playwright() as playwright:
        ...
        assert await page.input_value('#style-detail-mobilityScale') == '1'
        ...
        assert await page.input_value('#difficulty-detail-wldPreExactEmpties') == '0'
        ...
        assert '적용되지 않습니다' in style_note
        search_note = (await page.locator('#search-algorithm-note').text_content()) or ''
        assert '메인 탐색에 적용되지 않습니다' in search_note
        assert await page.locator('#difficulty-detail-button').is_disabled() is True
        assert await page.locator('#style-detail-button').is_enabled() is True
        await page.locator('#style-detail-button').click()
        await page.wait_for_timeout(50)
