...
## 저장소 메타데이터와 문서 기준선
- 현재 저장소 Stage: **Stage 147** (`stage-info.json` 기준)
...
- 평가: balanced13 phase-bucket linear evaluator + baseline linear move-ordering replay + diagonal compact tuple residual + runtime MPC generated module
...
- compact tuple additive lane 상태: Stage 124→147 follow-up에서 `diagonal-top24-latea-endgame + baseline ordering` stack이 final adoption gate와 post-adoption validation을 모두 통과해 **현재 active runtime 기본 generated module로 설치**되었습니다. pre-switch baseline은 `tools/engine-match/fixtures/historical-installed-modules/active-precompact-tuple.learned-eval-profile.generated.js`에 archive되어 있습니다.
- offline learning bundle 상태: Stage 126 weight-learning → Stage 144 confirmation → Stage 145 move-ordering compatibility replay → Stage 146 final adoption gate → Stage 147 install/post-adoption validation 순서로 compact tuple lane을 재검증했습니다. 현재 active runtime은 `benchmarks/stage146/selected-final-generated-module.js`와 byte-level parity를 유지하고, 설치본 snapshot은 `benchmarks/stage147/installed-active-generated-module.js`로 고정했습니다.
