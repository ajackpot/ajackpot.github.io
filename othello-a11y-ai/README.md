...
- `설정`과 `사용 중인 엔진 수치` 영역을 버튼으로 접고 펼칠 수 있도록 구성
- `사용자 지정`일 때만 직접 입력한 엔진 수치가 실제 적용되도록 제어
- `사용자 지정`일 때는 스타일 프리셋을 비활성화하고 직접 입력값을 우선 적용
...
- 사용자 지정 프리셋에서는 스타일 프리셋이 비활성화되고 직접 입력값이 우선 적용됨
- `쉬움`, `불가능` 난이도 프리셋 추가
- 보드 64칸 모두 버튼으로 제공됨
- 접근 가능한 이름 형식 정상
...
## 이번 단계에서 추가한 점
- `설정` 영역을 버튼으로 접고 펼칠 수 있게 바꾸고, 기본 상태를 접힘으로 조정했습니다.
- 설정이 접혀 있어도 `현재 설정으로 새 게임 시작`, `직전 사람 차례로 무르기`, `현재 상태 다시 읽기` 버튼은 항상 보이도록 분리했습니다.
- `사용 중인 엔진 수치` 영역도 별도 버튼으로 접고 펼칠 수 있게 바꾸고, 기본 상태를 접힘으로 조정했습니다.
- `쉬움` 난이도를 추가해 `입문`과 `보통` 사이의 완만한 상승 구간을 만들었습니다.
- `불가능` 난이도를 추가해 10초 이상 생각하는 브라우저 내 최강 옵션을 제공했습니다.
- 사용자 지정 프리셋에서는 스타일 프리셋이 비활성화되고, 직접 입력한 수치가 그대로 적용되도록 바꿨습니다.
