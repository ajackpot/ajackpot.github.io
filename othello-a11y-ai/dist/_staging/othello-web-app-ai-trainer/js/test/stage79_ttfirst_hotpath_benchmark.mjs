...
const DEFAULT_OUTPUT_PATH = path.join(PROJECT_ROOT, 'benchmarks', 'stage79_ttfirst_hotpath_benchmark.json');

const DEPTH_LIMITED_SEEDS_20 = [11, 29, 47, 83, 101, 137];
...
function withToggle(options, enabled) {
  return {
    ...options,
    ttFirstInPlaceMoveExtraction: enabled,
  };
}
...
function buildSummary(cases) {
  ...
  const baselineTtFirstSearches = sumBy(cases.map((entry) => entry.baseline), 'ttFirstSearches');
  const candidateTtFirstSearches = sumBy(cases.map((entry) => entry.candidate), 'ttFirstSearches');
  const baselineTtFirstCutoffs = sumBy(cases.map((entry) => entry.baseline), 'ttFirstCutoffs');
  const candidateTtFirstCutoffs = sumBy(cases.map((entry) => entry.candidate), 'ttFirstCutoffs');
  ...
  return {
    ...
    baselineTtFirstSearches,
    candidateTtFirstSearches,
    baselineTtFirstCutoffs,
    candidateTtFirstCutoffs,
  };
}
...
async function main() {
  ...
  const output = {
    benchmark: 'Stage 79 TT-first hotpath cleanup benchmark',
    description: 'Compare the legacy array-copy TT-first preferred-move extraction against an in-place remaining-move compaction path using the same engine and identical runtime options.',
    depthLimitedSection,
    wld14Section,
    exact14Section,
    exact10Section,
