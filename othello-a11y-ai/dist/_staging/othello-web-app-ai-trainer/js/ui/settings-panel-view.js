...
export class SettingsPanelView {
  ...
  buildMarkup(initialSettings) {
    ...
    `).join('');
    const selectedSearchAlgorithm = describeSearchAlgorithm(normalizedSearchAlgorithm);
    const searchAlgorithmNote = [
      selectedSearchAlgorithm?.description ?? '',
      describeSearchAlgorithmAvailability(initialSettings.presetKey),
    ].filter(Boolean).join(' ');
            ...
            <section class="settings-group-panel" aria-labelledby="engine-settings-title">
              <div class="settings-group-panel-header">
                ...
                <p id="settings-help-text" class="subtle-text settings-group-panel-description">
                  난이도와 스타일은 즉시 반영됩니다. 다만 화면 읽기 낭독은 자동으로 다시 읽지 않으며, 필요하면 아래의 “현재 설정 값 다시 읽기” 버튼을 사용할 수 있습니다. “사용자 지정” 난이도에서는 아래 입력값이 우선하며 스타일 프리셋은 잠시 비활성화됩니다.
                </p>
              </div>
              ...
              <fieldset>
                ...
                <p id="search-algorithm-note" class="subtle-text">${escapeHtml(searchAlgorithmNote)}</p>
              </fieldset>
  ...
  readSettings() {
    ...
    return {
      ...
      searchAlgorithm: this.searchAlgorithmSelect?.value || 'classic',
  ...
  syncSearchAlgorithmDescription() {
    ...
    const description = describeSearchAlgorithm(this.searchAlgorithmSelect.value)?.description ?? '';
    const availability = describeSearchAlgorithmAvailability(this.presetSelect.value || 'normal');
    this.searchAlgorithmNote.textContent = [description, availability].filter(Boolean).join(' ');
  }
