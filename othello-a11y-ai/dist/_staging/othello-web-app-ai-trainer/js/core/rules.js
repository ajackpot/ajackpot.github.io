...
function createPreparedSearchMoveRecord(moveBit, player, opponent) {
  let flips = 0n;
  let flipCount = 0;
...
export function listPreparedSearchMoves(player, opponent) {
  const legalMoves = legalMovesBitboard(player, opponent);
  if (legalMoves === 0n) {
    return [];
  }

  const details = new Array(popcount(legalMoves));
  let slot = 0;
  ...
  while (cursor !== 0n) {
    ...
    cursor ^= moveBit;
    details[slot] = createPreparedSearchMoveRecord(moveBit, player, opponent);
    slot += 1;
  }
