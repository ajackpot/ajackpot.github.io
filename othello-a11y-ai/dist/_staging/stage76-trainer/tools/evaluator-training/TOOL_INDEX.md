...
> 현재 정리 기준의 기본 결론: **브라우저 기본값은 baseline runtime 유지**입니다. tuple lane은 `run-multi-candidate-training-suite`와 `run-tuple-patch-suite`, MPC lane은 `run-mpc-candidate-training-suite` 중심으로 이어갑니다. `entryScales`는 마지막 미세 패치용입니다.
