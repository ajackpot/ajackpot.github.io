...
- `사용자 지정` 난이도에서만 root WLD pre-exact `+2` 창을 명시적으로 켜고 끌 수 있는 선택형 옵션 제공
...
- Stage 83에서는 `wldPreExactEmpties=2`를 더 이상 강한 프리셋에서 자동 활성화하지 않고, `사용자 지정` 난이도에서만 `사용 안 함 / +2 사용`으로 고를 수 있는 선택형 옵션으로 내렸습니다.
- 같은 단계에서 사용자 지정 설정 렌더링을 숫자 입력 전용에서 typed control(number/select) 기반으로 정리했고, `wldPreExactEmpties` 변경은 전이표 의미가 달라지므로 즉시 TT를 비우도록 안전 장치를 추가했습니다.
