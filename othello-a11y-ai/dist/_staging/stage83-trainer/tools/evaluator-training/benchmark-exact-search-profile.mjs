...
function printUsage() {
  const toolPath = displayTrainingToolPath('benchmark-exact-search-profile.mjs');
  ...
  const outputJsonPath = displayProjectPath('benchmarks', 'stage29_exact_profile_benchmark.json');
...
function updateAggregate(aggregate, baseline, candidate) {
  aggregate.cases += 1;
  aggregate.exactCases += baseline.exact && candidate.exact ? 1 : 0;
  aggregate.identicalScoreCases += baseline.score === candidate.score ? 1 : 0;
...
function finalizeAggregate(aggregate) {
  return {
    ...aggregate,
    nodeRatio: aggregate.baselineNodes > 0 ? aggregate.candidateNodes / aggregate.baselineNodes : null,
    elapsedRatio: aggregate.baselineElapsedMs > 0 ? aggregate.candidateElapsedMs / aggregate.baselineElapsedMs : null,
...
console.log('\n[overall]');
console.log(`cases=${summary.overall.cases}, exact=${summary.overall.exactCases}`);
console.log(`same score=${summary.overall.identicalScoreCases}/${summary.overall.cases}, same best move=${summary.overall.identicalBestMoveCases}/${summary.overall.cases}`);
